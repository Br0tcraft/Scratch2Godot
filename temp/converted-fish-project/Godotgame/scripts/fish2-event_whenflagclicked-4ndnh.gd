...
func _init_after_ready():
	...
	if str(goTo) == "_mouse_":
		...
		if target_node and target_node is Node2D:
			...
			correctur.print_not_existing("res://scripts/fish2-event_whenflagclicked-4ndnh.gd", "go to ()", str(goTo))

	secs = correctur.ms("1", "float", "res://scripts/fish2-event_whenflagclicked-4ndnh.gd", "wait (!) secs")
	...
	while true:
		...
		if str(lookat) == "_mouse_":
			...
			if lookat and lookat is Node2D:
				...
				correctur.print_not_existing("res://scripts/fish2-event_whenflagclicked-4ndnh.gd", "point towards (!)", str(lookat))
		...
		times = correctur.ms(op.rand("3","13"), "float", "res://scripts/fish2-event_whenflagclicked-4ndnh.gd", "repeat (): {}")
		for _j in int(times):

			steps = correctur.ms(op.rand("3","5"), "float", "res://scripts/fish2-event_whenflagclicked-4ndnh.gd", "move steps (!)")
			object.position += Vector2(cos(deg_to_rad(object.direction - 90)), sin(deg_to_rad(object.direction - 90))) * steps

			await get_tree().process_frame

		secs = correctur.ms(op.rand("0.05","1"), "float", "res://scripts/fish2-event_whenflagclicked-4ndnh.gd", "wait (!) secs")
		await get_tree().create_timer(secs).timeout
